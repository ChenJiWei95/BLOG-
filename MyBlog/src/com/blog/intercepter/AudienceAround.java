...
import com.blog.anno.RCacheEvict;
import com.blog.anno.RCachePut;
import com.blog.anno.RCacheable;
import com.blog.anno.RParamer;
...
public class AudienceAround {
	...
	@Pointcut("execution(* com.blog.service.impl.*.*(..)) || "
			+ "execution(* com.blog.control.admin.AdministratorsControl.setWebsite(..))")
  	public void performance(){
		log.info("判断是否使用redis缓存注解");
	}
  	@Around("performance()")
...
//	    	java.lang.NoSuchMethodException
	    	...
	    	log.info("返回值类型："+method.getReturnType().getSimpleName()); // 结果：Object 
	    	...
	    	for (Annotation annotation : annotations) {
				...
				if (annotation instanceof RCacheable) {
					...
					if(redisService.hasKey(key)) {
						...
						result = "".equals(rCacheable.result()) 
								? joinPoint.proceed() 
								: key_(rCacheable.result(), createOgnl(joinPoint, method));	
						// 根据不同的类返回型进行写入
						log.info("载入缓存，key："+key);
						redisService.set(key, result, rCacheable.time());
					}
					...
					result = "".equals(rCachePut.result()) 
							? joinPoint.proceed() 
							: key_(rCachePut.result(), createOgnl(joinPoint, method));	
					redisService.set(key, result, rCachePut.time());
				} 
			} 
